...
import { ShellLayout } from './shared/layout/shell-layout/shell-layout';
...
export const routes: Routes = [
  ...
  // Everything below is a CHILD of the shell, which is a route of its own now
  // rather than something each screen renders for itself. The shell is built
  // once and the router swaps only what is inside it - see ShellLayout for why
  // that matters.
  //
  // The guards do NOT move up here. Each screen guards on the code ITS OWN
  // data needs, and they differ (authGuard for the read screens,
  // permissionGuard for the rest); hoisting them would either shut people out
  // of screens they may use or let them reach ones they may not. ShellLayout
  // itself carries no guard: every child has one, so there is no way to land
  // on the shell alone.
  {
    path: '',
    ...
    children: [
      ...
      // nav entry does - which is the invariant NAV_GROUPS is built around.
      //
      ...
      // `manage_costs`, and it is the whole gate: every cost endpoint - the
      // `costs` read and the `farmCycles` picker included - is that code (V24,
      // OWNER and FARM_MANAGER). COMPANY-WIDE like Assets: the selected farm does
      // not narrow the register.
      {
        path: 'costs',
        component: Costs,
        canActivate: [permissionGuard(PERMISSION.MANAGE_COSTS)],
      },
      // `view_finance`, and it is the whole gate: both profitability queries
      // are that code (OWNER and FARM_MANAGER). The farm is an ARGUMENT, so
      // any of the caller's farms can be reported on. The one read inside it
      // on another code - the `farmCycles` picker, `manage_costs` - is gated
      // on the control itself (see Profit).
      {
        path: 'profit',
        component: Profit,
        canActivate: [permissionGuard(PERMISSION.VIEW_FINANCE)],
      },
      // authGuard and no permission: every account has a profile, including
      // one with no farm and no role. `PUT /api/auth/me` takes the user from
      // the token, so there is nothing here anyone could use on someone else.
      // Not in the nav - it is reached from the sidebar footer.
